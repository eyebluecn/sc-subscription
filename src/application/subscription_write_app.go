...
type SubscriptionWriteApp struct{}

func NewSubscriptionWriteApp() *SubscriptionWriteApp {
	return &SubscriptionWriteApp{}
}

// 准备订阅
func (receiver SubscriptionWriteApp) PrepareSubscribe(ctx context.Context, readerId int64, columnId int64, payMethod string) (*info_model.PrepareSubscribeInfo, error) {
	...
	//3. 下发一个准备单
	prepareSubscribeInfo, err := domain.NewOrderDomainService().CreateAndPrepare(ctx, *columnVO, *readerVO, payMethod)
	if err != nil {
		return nil, err
	}
...
// 支付单已完成支付。
func (receiver SubscriptionWriteApp) PaymentPaid(ctx context.Context, paymentId int64) (*model.Subscription, error) {
	...
	{
		//订单标记为已支付。
		order, err = domain.NewOrderDomainService().OrderPaid(ctx, order)
		if err != nil {
			return nil, err
		}

		//创建订阅关系。
		subscription, err = domain.NewSubscriptionDomainService().Create(ctx, order)
		if err != nil {
			return nil, err
		}
