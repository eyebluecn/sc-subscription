...
type ICommonResponse interface {
	GetBaseResp() *sc_subscription_base.BaseResp
	SetBaseResp(*sc_subscription_base.BaseResp)
}
...
func FinalWrapResponse(ctx context.Context, resp ICommonResponse, newResp ICommonResponse, err error) ICommonResponse {
	...
	if err != nil {
		...
		newResp.SetBaseResp(&sc_subscription_base.BaseResp{StatusCode: 500, StatusMessage: err.Error()})
		return newResp
	} else {

		if resp == nil {
			klog.CtxErrorf(ctx, "返回体不能为空")
			newResp.SetBaseResp(&sc_subscription_base.BaseResp{StatusCode: 500, StatusMessage: "返回体异常，不能为空"})
			return newResp
		}
		...
		if resp.GetBaseResp() == nil {
			//没有设置标准的返回体，将其设置上。
			resp.SetBaseResp(&sc_subscription_base.BaseResp{})
		}
